...
TerrainEditor::TerrainEditor(UINT width, UINT height)
    ...
    adjustValue(10.0f), editType(1), selectMap(0)
{
...
{
    ImGui::TextColored(ImVec4(1, 1, 0, 1), "TerrainEditor");

    const char* brushList[] = { "Circle", "Rectangle" };
...
void TerrainEditor::CreateMesh()
{
    ...
    for (UINT z = 0; z < height; z++)
    {
        for (UINT x = 0; x < width; x++)
        {
            ...
            vertex.position.y = pixels[index].x * MAX_HEIGHT + MIN_HEIGHT;
